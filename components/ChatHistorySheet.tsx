...
import { Plus, MoreVertical, Pencil, Trash2, Pin, Search, X, CheckSquare, Square } from 'lucide-react';
...
const ChatHistorySheet: React.FC<ChatHistorySheetProps> = ({
    ...
    const handleConfirmDeleteMultiple = () => {
        onDeleteMultipleConversations(selectedIds);
                ...
                <div className="px-4 pb-4 border-b border-neutral-200 dark:border-gray-700 flex-shrink-0">
                    <div className="flex items-center justify-between mb-3">
                        <h2 id="history-sheet-title" className="text-lg font-semibold text-neutral-800 dark:text-gray-200">
                             {selectionMode ? `Selected (${selectedIds.length})` : 'Chat History'}
                        </h2>
                         <div className="flex items-center gap-2">
                            {selectionMode ? (
                                <>
                                    <button onClick={() => { setSelectionMode(false); setSelectedIds([]); }} className="px-3 py-1.5 text-sm font-semibold rounded-lg text-neutral-700 dark:text-gray-200 hover:bg-neutral-100 dark:hover:bg-gray-700 transition-colors">Cancel</button>
                                    <button onClick={() => setIsDeleteConfirmOpen(true)} disabled={selectedIds.length === 0} className="px-3 py-1.5 text-sm font-semibold rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Delete</button>
                                </>
                            ) : (
                                <>
                                    {conversations.length > 0 && <button onClick={() => setSelectionMode(true)} className="px-3 py-1.5 text-sm font-semibold rounded-lg text-neutral-700 dark:text-gray-200 hover:bg-neutral-100 dark:hover:bg-gray-700 transition-colors">Select</button>}
                                    <button
                            ...
                            {filteredConversations.map((convo) => {
                                ...
                                <li key={convo.id} className="group relative">
                                    {renamingId === convo.id ? (
                                        ...
                                        <button
                                            onClick={() => selectionMode ? handleToggleSelection(convo.id) : (onSelectConversation(convo.id), onClose())}
                                            className={`w-full text-left p-3 my-1 rounded-lg transition-colors flex items-start gap-3 ${
                                                convo.id === activeConversationId && !selectionMode
                                        ...
                                        >
                                            ...
                                            {convo.isPinned && !selectionMode && <Pin className="h-4 w-4 text-amber-500 dark:text-amber-400 transform -rotate-45 flex-shrink-0 mt-1" />}
                                            <div className="flex-1 min-w-0">
                                                {convo.isGeneratingTitle ? (
                                                     <div className="w-3/4 h-4 bg-neutral-200 dark:bg-gray-700 rounded-md animate-pulse"></div>
                                                ) : (
                                                     <span className={`font-medium break-words ${isSelected ? 'text-amber-700 dark:text-amber-300' : ''}`}>{convo.title}</span>
                                                )}
                                    ...
                                    {!selectionMode && renamingId !== convo.id && (
