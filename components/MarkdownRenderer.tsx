...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, sources, onContentUpdate, isStreaming, setCodeForPreview }) => {
    ...
    // A more robust check for a markdown table separator line.
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (i > 0 && isTableSeparator(line)) {
            const headerLine = lines[i - 1];
            // Check if the previous element was a paragraph containing the header, and remove it.
            if (elements.length > 0 && elements[elements.length - 1].key === `${i - 1}`) {
                elements.pop();
            }
            flushList(`list-before-table-${i}`);
            
            const separatorLine = line;
            ...
            const alignments = parseRow(separatorLine).map(s => {
            ...
            while (tableIndex < lines.length && lines[tableIndex].includes('|')) {
                tableRows.push(parseRow(lines[tableIndex]));
                tableIndex++;
            }
            
            elements.push(
                <div key={`table-container-${i}`} className="overflow-x-auto">
                    <table className={`dynamic-table ${getRandomTableStyle()}`}>
                        <thead>
                            <tr>
                                {headers.map((header, index) => (
                                    <th key={index} style={{ textAlign: alignments[index] as any }}>
            ...
            i = tableIndex - 1; // Move index past the processed table lines
            continue;
        }
        ...
        if (headingMatch) {
            ...
            elements.push(<Tag key={i} className={classNames}>{parseInline(text, sources)}</Tag>);
            continue;
        }
        ...
        if (line.trim() !== '') {
            elements.push(<p key={i}>{parseInline(line, sources)}</p>);
