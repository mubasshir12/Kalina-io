...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, onSetApiKey, onClose, currentApiKey }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={!currentApiKey ? undefined : onClose}>
      <div className="w-full sm:max-w-md bg-white dark:bg-[#1e1f22] rounded-2xl shadow-xl p-8 space-y-6" onClick={(e) => e.stopPropagation()}>
